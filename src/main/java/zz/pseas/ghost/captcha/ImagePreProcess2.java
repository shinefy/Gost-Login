...
public class ImagePreProcess2 {
	...
	public static String getAllOcr(String file) throws Exception {
		...
		ImageIO.write(img, "JPG", new File("resources\\result2\\" + result + ".jpg"));
		return result;
	}
	...
	public static void downloadImage() {
		...
		for (int i = 0; i < 30; i++) {
			...
			try {
				...
				// ��ȡ����
				String picName = "resources\\img2\\" + i + ".jpg";
	...
	public static void trainData() throws Exception {
		File dir = new File("resources\\temp");
		File[] files = dir.listFiles();
		for (File file : files) {
			BufferedImage img = removeBackgroud("resources\\temp\\" + file.getName());
			List<BufferedImage> listImg = splitImage(img);
			if (listImg.size() == 4) {
				for (int j = 0; j < listImg.size(); ++j) {
					ImageIO.write(listImg.get(j), "JPG", new File("resources\\train2\\"
